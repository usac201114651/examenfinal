#CATC importamos las clases necesarias
import threading 
import logging
import time
import os 
...
#CATC creamos un objeto tipo cliente, con atributos del mqtt
cli = Mqtt("proyectos", "proyectos980", "167.71.243.238", 1883)
com = Comandos()
aud = Sonido()
#CATC bucle para enviar alives al servidor por mqtt
def alive(delay = com.delay_a()):
    while True:
        alive  = com.command_alive()  +  bytes(str(cli.carnet()), 'utf-8')
        cli.client.publish(str(cli.topicalive()), alive)
        time.sleep(delay)          
#CATC iniciamos el hilo para que lo envíe en el fonoo y muera cuando muera el cliente
...
#CATC desconectar y matar los hilos
